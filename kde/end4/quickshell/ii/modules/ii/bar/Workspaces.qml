...
Item {
    ...
    property int effectiveWorkspaceCount: 1
    ...
    Process {
        id: kwinDesktopCountPoller
        running: true
        command: ["qdbus6", "org.kde.KWin", "/VirtualDesktopManager", "count"]
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                if (!isNaN(val) && val > 0) {
                    root.effectiveWorkspaceCount = val;
    ...
    Process {
        id: kwinDesktopCountListener
        running: true
        command: ["dbus-monitor", "type='signal',interface='org.kde.KWin.VirtualDesktopManager',member='countChanged'"]

        stdout: StdioCollector {
            waitForEnd: false

            onDataChanged: {
                kwinDesktopCountPoller.running = true;
            }
        }
    }

    readonly property int workspacesShown: effectiveWorkspaceCount
    ...
    Grid {
        ...
        Repeater {
			...
			MouseArea {
                            ...
                            onPressed: event => {
                                if (event.button === Qt.RightButton && workspaceButtonBackground.biggestWindow?.class) {
                                    TaskbarApps.togglePin(workspaceButtonBackground.biggestWindow.class);
                                    event.accepted = true;
                                }
                            }
                        }
